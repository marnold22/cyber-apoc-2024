# Fake-Boost

## FLAG: HTB{}

## Status: Incomplete

+ DOCKER: No
+ DOWNLOADABLE: Yes

Description: In the shadow of The Fray, a new test called ""Fake Boost"" whispers promises of free Discord Nitro perks. It's a trap, set in a world where nothing comes without a cost. As factions clash and alliances shift, the truth behind Fake Boost could be the key to survival or downfall. Will your faction see through the deception? KORP™ challenges you to discern reality from illusion in this cunning trial.

## NOTES

1. Extract files
   1. > unzip forensics_fake_boost.zip
      1. capture.pcapng
2. WIRESHARK
   1. > wireshark capture.pcapng
   2. /GET freediscordnitro

        ```text
            $jozeq3n = "" ;
            $s0yAY2gmHVNFd7QZ = $jozeq3n.ToCharArray() ; [array]::Reverse($s0yAY2gmHVNFd7QZ) ; -join $s0yAY2gmHVNFd7QZ 2>&1> $null ;
            $LOaDcODEoPX3ZoUgP2T6cvl3KEK = [sYSTeM.TeXt.ENcODING]::UTf8.geTSTRiNG([SYSTEm.cOnVeRT]::FRoMBaSe64sTRing("$s0yAY2gmHVNFd7QZ")) ;
            $U9COA51JG8eTcHhs0YFxrQ3j = "Inv"+"OKe"+"-EX"+"pRe"+"SSI"+"On" ; New-alIaS -Name pWn -VaLuE $U9COA51JG8eTcHhs0YFxrQ3j -FoRcE ; pWn $lOADcODEoPX3ZoUgP2T6cvl3KEK ;
        ```

        1. We need to deobfuscate this
        2. Looking at it we see a few key pieces
3. DEOBFUSCATE
   1. First the strings is converted to a character array
   2. Second the array is reversed and joined
   3. Third we run convert from BASE64 on the string
   4. Fourth we run InvokeExpression - which is a powershell command 
4. PYTHON
   1. Run our python script and we get the base64 decoded payload which looks to be a powershell script
5. POWERSHELL
   1. Cleaning up the code we see
   2. $part1 = "SFRCe2ZyMzNfTjE3cjBHM25fM3hwMDUzZCFf"
      1. FROM BASE64 -> `HTB{fr33_N17r0G3n_3xp053d!_`
      2. First part of the flag
6. WIRESHARK PT.2
   1. Going back to wireshark we see a /POST request with the response

      ```text
         
      ```

      1. This looks like base64 encoded data
      2. Testing and it doesn't comeback with legible content, so maybe since it is a POST request, this uses our AES key we saw earlier
7. AES DECODE
   1. Encoded Data: ""
   2. Key Size: 256bit
   3. AES-Key: "Y1dwaHJOVGs5d2dXWjkzdDE5amF5cW5sYUR1SWVGS2k=" -> FROM BASE64 -> "cWphrNTk9wgWZ93t19jayqnlaDuIeFKi"
   4. DECODED

      ```text
         [
            {
               "ID":  "1212103240066535494",
               "Email":  "YjNXNHIzXzBmX1QwMF9nMDBkXzJfYjNfN3J1M18wZmYzcjV9",
               "GlobalName":  "phreaks_admin",
               "Token":  "MoIxtjEwMz20M5ArNjUzNTQ5NA.Gw3-GW.bGyEkOVlZCsfQ8-6FQnxc9sMa15h7UP3cCOFNk"
            },
            {
               "ID":  "1212103240066535494",
               "Email":  "YjNXNHIzXzBmX1QwMF9nMDBkXzJfYjNfN3J1M18wZmYzcjV9",
               "GlobalName":  "phreaks_admin",
               "Token":  "MoIxtjEwMz20M5ArNjUzNTQ5NA.Gw3-GW.bGyEkOVlZCsfQ8-6FQnxc9sMa15h7UP3cCOFNk"
            }
         ]
      ```

      1. Now we have potential tokens
         1. TOKEN -> FROM BASE64 -> `b3W4r3_0f_T00_g00d_2_b3_7ru3_0ff3r5}`
8. FLAG
   1. Now we have both parts lets combine them
      1. FLAG: `HTB{fr33_N17r0G3n_3xp053d!_b3W4r3_0f_T00_g00d_2_b3_7ru3_0ff3r5}`